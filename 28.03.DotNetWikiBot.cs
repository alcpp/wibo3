// DotNetWikiBot Framework v1.3 - bot framework based on Microsoft® .NET Framework 2.0 for wiki projects
...
{	
	...
	public class Site
	{
		...
		public Site(bool create)
		{
			if (create == true) throw new Exception("asdasdasd");
				//this.Site();
	...
	public class Page
	{
		public static readonly string CacheDirectory = "PageCache\\";
		static Page()
		{
		...
		/*bool TryToLoadFromFile(string pageName)
		{
		...
		public void Load()
		{
		    ...
		    Console.WriteLine("Page \"" + title + "\" loaded successfully in "+ ts.ToString());

			SaveToFile();
			SaveToFile(CacheDirectory + title);
		}
		...
		public void ParsePageXML(string xmlSrc)
	    {
			...
			TimeSpan ts = DateTime.Now - begin;
		    Console.WriteLine("Page \"" + title + "\" loaded successfully in " + ts.ToString() + "seconds");
	    }
	    ...
	    {
			Console.WriteLine("Saving page" + title);
			DateTime begin = DateTime.Now;
			GetEditSessionData();
			...
			if (site.editSessionTokenRE1.IsMatch(respStr))
				...
				Console.WriteLine("Page \"" + title + "\" saved successfully in " + ts.Seconds + "seconds");
		    	text = newText;
	    	}
			SaveToFile();
			SaveToFile(CacheDirectory + title);
	    ...
	    public void UploadImage(string filePathName, string description, string license)
	    {
            ...
            strmReader.Close();
            webResp.Close();
			if (respStr.Contains(site.messages["MediaWiki:Uploadcorrupt"].text) == true ||
				respStr.Contains(fileName) == false)
				Console.WriteLine("Error occured when uploading image \"" + filePathName + "\".");   		
   			else {
		    	Console.WriteLine("Image \"" + filePathName + "\" uploaded successfully.");
		...
		/// <param name="license">Image license type. Used only in some wiki sites. Pass
		/// empty string, if the wiki site doesn't require it.</param>						
	    public void UploadImageFromWeb(string imageFileUrl, string description, string license)
	    {		    
		    ...
		    UploadImage("Cache\\" + imageFileName, description, license);
	    ...
	    {		    
		    Uri res = new Uri(site.site + site.wikiPath + title);
		    Bot.InitWebClient();
		    string src = "";
		    try {
		    ...
		    //string m = site.messages["MediaWiki:Showbigimage"].text;
		    //m = m.Substring(0, m.IndexOf("$1")-1);
		    Regex imageLinkRE = new Regex("<a href=\"([^\"]+?)\" class=\"internal\"");
	...
	{
		public static readonly string LinksToPageCache = Page.CacheDirectory + "LinksToPage\\";
		static PageList()
		{
			Directory.CreateDirectory(LinksToPageCache);
		}
		...
		/// <summary>This index allows to call pageList["title"].</summary>
		...
		/// "Special:Recentchanges". Image uploads are not included, use the log instead.
		...
		public void FillFromLinksToPage(string pageTitle)
		{
			...
			if (File.Exists(LinksToPageCache + pageTitle))
			{
				this.FillFromFile(LinksToPageCache + pageTitle);
				return;
			}
			...
			Console.WriteLine("PageList filled with titles of pages, referring to \"" +
				pageTitle + "\".");
			this.SaveTitlesToFile(LinksToPageCache + pageTitle);
		}

		/// <summary>Gets page titles of pages, in which the specified image is included.</summary>
		/// <param name="imageFileTitle">Image file title. Without "Image:" prefix.</param>	
		public void FillFromPagesUsingImage(string imageFileTitle)
		{			
			Uri res = new Uri(site.site + site.wikiPath + "Image:" + imageFileTitle);
		    Bot.InitWebClient();
		    string src = Bot.wc.DownloadString(res);
		...
		{
			HttpWebRequest webReq = (HttpWebRequest)WebRequest.Create(site.site + 
	        	site.indexPath + "index.php?title=Special:Watchlist/edit");
            webReq.Proxy.Credentials = CredentialCache.DefaultCredentials;
	        webReq.ContentType = Bot.webContentType;
            webReq.UserAgent = Bot.botVer;
            webReq.CookieContainer = new CookieContainer();
		...
		public void FillFromGoogleSearchResults(string searchStr, int limit)
		{
		    ...
		    foreach (Match match in matches)
			    pages.Add(new Page(site,
			    	HttpUtility.UrlDecode(match.Groups[1].Value).Replace("_", " ")));
		...
		/// <summary>Loads text for pages in PageList from site via common web interface. It seems it is much slower, than loadEx</summary>		
		public void Load()
		{
		...
		public void LoadEx()
		{
			...
			foreach (Page page in pages)
			{
				...
				{
					page.SaveToFile();
					page.SaveToFile(Page.CacheDirectory + page.title);
				}
			}
			TimeSpan ts = DateTime.Now - begin;
			Console.WriteLine("Pages download completed in " + ts.ToString() );
		}
	...
	{	
		...
		public static string botVer = "DotNetWikiBot/1.3";
